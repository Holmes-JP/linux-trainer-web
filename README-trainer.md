...
**公開 URL: https://holmes-jp.github.io/linux-trainer-web/** (リポジトリは 2026-07-04 に Public 化済み)

`.github/workflows/pages.yml` を **手動発火 (workflow_dispatch)** すると `npm run build` → `build/` を Pages に公開する。
注意:

- サブパス配信 (`github.io/linux-trainer-web/`) のため、workflow が build 時に `BASE_PATH=/linux-trainer-web` を渡し、
  `svelte.config.js` の `kit.paths.base` が拾う。アプリ内リンク/fetch は `$app/paths` の `base` 経由。
  **独自ドメイン (root 配信) に移す場合は workflow の BASE_PATH を外すだけでよい**。
